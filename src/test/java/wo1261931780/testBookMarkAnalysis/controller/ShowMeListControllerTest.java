 ...
 * Package:wo1261931780.testBookMarkAnalysis.controller
 *
 * @author liujiajun_junw @Date 2026-01-04 @Description 书签控制器集成测试
 */
@SpringBootTest
...
class ShowMeListControllerTest {
    ...
    void testShowMeList() throws Exception {
        mockMvc.perform(get("/BookMarks/list").param("page", "1").param("limit", "10"))
    ...
    void testShowMeListInvalidPage() throws Exception {
        mockMvc.perform(get("/BookMarks/list").param("page", "0").param("limit", "10"))
    ...
    void testShowMeListInvalidLimit() throws Exception {
        mockMvc.perform(get("/BookMarks/list").param("page", "1").param("limit", "101"))
                .andExpect(status().isBadRequest());
    }
    ...
    void testExportHtml() throws Exception {
        mockMvc.perform(get("/BookMarks/export").param("format", "html"))
                .andExpect(status().isOk())
                .andExpect(
                        header().string(
                                        "Content-Disposition",
    ...
    void testExportMarkdown() throws Exception {
        mockMvc.perform(get("/BookMarks/export").param("format", "markdown"))
                .andExpect(status().isOk())
                .andExpect(
                        header().string(
                                        "Content-Disposition",
    ...
    void testExportJson() throws Exception {
        mockMvc.perform(get("/BookMarks/export").param("format", "json"))
                .andExpect(status().isOk())
                .andExpect(
                        header().string(
                                        "Content-Disposition",
