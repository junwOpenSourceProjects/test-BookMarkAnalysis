 ...
 * Package:wo1261931780.testBookMarkAnalysis.service.impl
 *
 * @author liujiajun_junw @Date 2026-01-04 @Description 链接检测服务实现类
 */
@Slf4j
@Service
public class LinkCheckServiceImpl implements LinkCheckService {
    ...
    public LinkCheckReport checkLinks(List<String> urls) {
        ...
        // 使用并发检测
        List<CompletableFuture<LinkCheckResult>> futures =
                urls.stream()
                        .map(
                                url ->
                                        CompletableFuture.supplyAsync(
                                                () -> checkLink(url), executorService))
                        .collect(Collectors.toList());

        // 等待所有任务完成
        List<LinkCheckResult> results =
    ...
    @Override
    public LinkCheckReport checkAllLinksAsync() {
        String taskId = IdUtil.fastSimpleUUID();

        // 初始化任务进度
        LinkCheckReport initialReport =
                LinkCheckReport.builder()
                        .taskId(taskId)
                        .status("RUNNING")
                        .progress(0)
                        .totalChecked(0)
                ...
                () -> {
                    try {
                        ...
                        List<BookMarks> bookmarks =
                                bookmarksParserService.list().stream()
                                        .filter(
                                                b ->
                                                        "a".equals(b.getType())
                                                                && StrUtil.isNotEmpty(b.getHref()))
                                        .collect(Collectors.toList());
                        ...
                        for (int i = 0; i < bookmarks.size(); i++) {
                            ...
                            LinkCheckReport progressReport =
                                    LinkCheckReport.builder()
                                            .taskId(taskId)
                                            .status("RUNNING")
                                            .progress(progress)
    ...
    public LinkCheckReport checkAllLinks(int limit) {
        ...
        List<String> urls = bookmarks.stream().map(BookMarks::getHref).collect(Collectors.toList());

        LinkCheckReport report = checkLinks(urls);

        // 补充书签ID和标题信息
        Map<String, BookMarks> urlToBookmark =
                bookmarks.stream()
                        .collect(Collectors.toMap(BookMarks::getHref, b -> b, (a, b) -> a));

        report.getBrokenLinks()
                .forEach(
                        result -> {
    ...
    public LinkCheckReport getCheckProgress(String taskId) {
        ...
        if (report == null) {
            return LinkCheckReport.builder().taskId(taskId).status("NOT_FOUND").build();
        }
        return report;
    }
}
