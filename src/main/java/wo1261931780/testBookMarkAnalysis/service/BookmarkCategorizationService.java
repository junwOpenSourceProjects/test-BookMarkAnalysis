...
public class BookmarkCategorizationService {
    ...
    public List<Map<String, Object>> categorizeBookmarks(String apiBaseUrl, String apiKey, String modelName, List<Long> bookmarkIds) throws Exception {
        ...
        if (proxyHost != null && proxyPort != null && !proxyHost.isEmpty()) {
            try {
                request.setProxy(new java.net.Proxy(java.net.Proxy.Type.HTTP, new java.net.InetSocketAddress(proxyHost, Integer.parseInt(proxyPort))));
            } catch (Exception ignored) {}
        }
        
        try (cn.hutool.http.HttpResponse response = request.execute()) {
            if (!response.isOk()) {
                throw new RuntimeException("AI API 请求失败: HTTP " + response.getStatus() + " - " + response.body());
            }
            responseBodyStr = response.body();
        }
        JSONObject responseJson = JSONUtil.parseObj(responseBodyStr);
        String textReply = responseJson.getJSONArray("choices")
                                       .getJSONObject(0)
    ...
    public Map<String, Object> initReconstructTree() {
        ...
        List<BookMarks> allLinks = bookMarksService.list(
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<BookMarks>()
        ...
        Map<String, List<BookMarks>> groupedByDomain = allLinks.stream()
                .collect(Collectors.groupingBy(bm -> extractDomain(bm.getHref())));
        ...
        currentSession.totalBatches = (int) Math.ceil((double) allDomainProfiles.size() / currentSession.batchSize);
        ...
        fallback.setId(cn.hutool.core.util.IdUtil.getSnowflakeNextId());
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> fetchNextBatch(String apiBaseUrl, String apiKey, String modelName) throws Exception {
        if (currentSession == null || currentSession.currentBatchIndex >= currentSession.totalBatches) {
            return new ArrayList<>();
        }
        
        int fromIndex = currentSession.currentBatchIndex * currentSession.batchSize;
        int toIndex = Math.min(fromIndex + currentSession.batchSize, currentSession.allDomainProfiles.size());
        List<JSONObject> batchProfiles = currentSession.allDomainProfiles.subList(fromIndex, toIndex);

        JSONArray batchArray = new JSONArray();
        batchArray.addAll(batchProfiles);

        String systemPrompt = "你是一位资深的知识管理专家。\n" +
    ...
    public Map<String, Object> confirmBatch(List<Map<String, Object>> mappings) {
        ...
        for (Map<String, Object> mapObj : mappings) {
            ...
            if (folderName != null && !folderName.isEmpty() && !currentSession.folderDict.containsKey(folderName)) {
        ...
        for (String fName : newFolderNames) {
            ...
            fItem.setId(cn.hutool.core.util.IdUtil.getSnowflakeNextId());
        ...
        for (BookMarks link : currentSession.allLinks) {
            ...
            if (domainToFolderId.containsKey(domain)) {
                ...
                if (link.getParentId() == null || !link.getParentId().equals(currentSession.fallbackFolderId)) {
